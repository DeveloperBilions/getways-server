      ...
      if (results.length > 0) {
        ...
        for (const record of results) {
          ...
          if (transactionId) {
            ...
            record.set("status", 1);
            await record.save();
            console.log(`Transaction updated for recordId ${record.id} with new status 1`);
          } else {
            // If there is no transactionId and status is 0, delete the record
            await record.destroy();
            console.log(`Transaction record deleted for recordId ${record.id} due to missing transactionId`);
          }
        }
      } else {
